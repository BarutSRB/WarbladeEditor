...
| N04 | Redistribution of retail assets | Extracted content stays out of the repository and hash-pinned; it is never committed, pushed, or published as loose files. Compiled release builds embed it under the authorization from the original author recorded in `export_presets.cfg` (`application/copyright`), by user decision (2026-09-02). | Non-goal |
...
| I02 | Cross-platform delivery | Windows x86_64 and Linux x86_64 client builds ship alongside the macOS export from 0.1.0 (`make release-build`): unsigned, header-checked, and smoke-tested only headless in a linux/amd64 container. Cross-platform delivery beyond that (signing, input certification, and platform QA on real hardware) stays a separate program from the supported macOS exports. | Separate program |
